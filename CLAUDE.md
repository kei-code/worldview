# WorldView プロジェクト — Claude Code 設定

## プロジェクト概要

**WorldView** は、世界情勢を複数の国・立場の視点から図解・データで提示する地政学ニュースサイト。
情報の偏りをなくすことを目的とし、一つのトピックを当事国を含む複数の視点で整理する。

- **公開先**：GitHub Pages
- **リポジトリ**：https://github.com/kei-code/worldview
- **公開URL**：https://kei-code.github.io/worldview/
- **更新方式**：手動（都度指示）
- **表示言語**：日本語

---

## フォルダ構造

```
21-世界情勢情報収集/
├── index.html                        ← ホームページ
├── assets/
│   ├── css/
│   │   ├── common.css                ← 全ページ共通スタイル（色変数・ヘッダー・検索UI）
│   │   └── article.css               ← 記事ページ専用スタイル
│   ├── js/
│   │   ├── main.js                   ← ヘッダーナビ・タブ・アニメーション・TOCスクロール
│   │   └── search.js                 ← サイト内検索（main.jsが動的に読み込む）
│   └── data/
│       └── articles.js               ← 検索インデックス（生成物・手で編集しない）
├── articles/
│   └── {カテゴリ}/
│       └── {YYYY-MM-DD-スラッグ}/
│           └── index.html            ← 記事本体
├── categories/
│   └── {カテゴリ}/
│       └── index.html                ← カテゴリ一覧ページ
├── image/
│   ├── home/                         ← ホームのヒーロー・カテゴリ画像
│   ├── articles/                     ← 記事内で使用する画像（SEO対応ファイル名）
│   └── tmp/                          ← ユーザーが一時格納する作業フォルダ
└── tools/
    └── build_search_index.py         ← 検索インデックス生成スクリプト
```

---

## カテゴリ一覧

**カテゴリタグ自体は色分けしない**（全タグ中立グレー `class="tag"`）。下表の色は**記事のテーマ色（アクセント）**＝セクション番号・カレッジボックスの左罫・グラフ等に使う色。**テーマ色＝カテゴリ色を厳格に守る**（記事ごとに気分で変えない）。

色は「トピックのドメイン」を軸に5系統へ整理している：

| ドメイン（色の意味） | テーマ色 | 該当カテゴリ |
|---|---|---|
| 国際紛争・安全保障 | `--accent2` テラコッタ | 中東情勢・ウクライナ紛争・安全保障 |
| テクノロジー | `--accent-tech` 青（#80acd8） | AI・テクノロジー |
| 地域の勢力図 | `--accent5` パープル | アジア情勢・欧州情勢 |
| 資源・環境 | `--accent3` グリーン | 資源・環境 |
| 経済・社会・内政 | `--accent4` ベージュ/金 | 世界経済・移民政策・世論選挙 |

| カテゴリID | 表示名 | テーマ色 |
|---|---|---|
| middle-east | 中東情勢 | `--accent2`（テラコッタ） |
| ukraine | ウクライナ紛争 | `--accent2`（テラコッタ） |
| ai-tech | AI | `--accent-tech`（青 #80acd8） |
| technology | テクノロジー | `--accent-tech`（青 #80acd8） |
| immigration | 移民政策 | `--accent4`（ベージュ） |
| economy | 世界経済 | `--accent4`（ベージュ） |
| asia | アジア情勢 | `--accent5`（パープル） |
| europe | 欧州情勢 | `--accent5`（パープル） |
| resources | 資源・環境 | `--accent3`（グリーン） |
| security | 安全保障 | `--accent2`（テラコッタ） |
| elections | 世論・選挙 | `--accent4`（ベージュ） |

新規記事は、この表のテーマ色を `var(--accentN)` とグラフ等の直書き `rgba()` の両方に適用する。スタンスの色分けは tone-meter（`.tone-hostile`=赤／`.tone-supportive`=緑）で別途行い、テーマ色とは独立。

---

## 既存記事一覧（重複防止用）

| 日付 | カテゴリ | タイトル | スラッグ |
|---|---|---|---|
| 2026-06-02 | middle-east | イラン核交渉の最新動向 — 米国・イスラエル・イランの三つ巴 | 2026-06-02-iran-nuclear |
| 2026-06-02 | ukraine | ウクライナ停戦交渉の現在地 | 2026-06-02-ceasefire |
| 2026-06-02 | ai-tech | 米中AI覇権争いの現在地 | 2026-06-02-ai-race |
| 2026-06-02 | immigration | 移民政策の世界地図 — 締め出す国・呼び込む国 | 2026-06-02-immigration-policy |
| 2026-06-03 | economy | トランプ関税が変えた世界貿易地図 | 2026-06-03-trump-tariffs |
| 2026-06-04 | economy | 重要鉱物の囲い込み — レアアース・グリーンランド・コンゴの新争奪戦 | 2026-06-04-critical-minerals |
| 2026-06-04 | asia | インドの「全方位外交」— 米中どちらにも付かない14億人の大国 | 2026-06-04-india-strategy |
| 2026-06-04 | ai-tech | AIが変えた科学の速度 — AlphaFold以後の「発見の自動化」 | 2026-06-04-ai-science |
| 2026-06-05 | europe | 欧州の「自前防衛」への転換 — アメリカ抜きで大陸を守れるか | 2026-06-05-european-defense |
| 2026-06-06 | resources | 水の地政学 — 川とダムをめぐる「静かな戦争」 | 2026-06-06-water-geopolitics |
| 2026-06-06 | technology | 軌道の覇権争い — スターリンクと「宇宙インフラの私物化」 | 2026-06-06-orbital-supremacy |
| 2026-06-07 | economy | 脱ドルへの挑戦 — 基軸通貨は揺らぐのか | 2026-06-07-dedollarization |
| 2026-06-07 | security | 海底ケーブルの「影の戦争」 — 事故か、攻撃か | 2026-06-07-undersea-cables |
| 2026-06-07 | economy | エレクトロステートの誕生 — 「石油の国」から「電気の国」へ | 2026-06-07-electrostate |
| 2026-06-08 | security | 「右傾化する世界」 — ポピュリズムはなぜ民主主義を変えるのか | 2026-06-08-global-right-shift |
| 2026-06-09 | elections | 「2つのアメリカ」— トランプ支持率の解剖 | 2026-06-09-trump-approval |
| 2026-06-11 | middle-east | 核を手放した国の末路 — 「核抑止」という生存の論理 | 2026-06-11-nuclear-deterrence |
| 2026-06-12 | middle-east | 火の輪は、なぜ消えたか — イラン「抵抗の枢軸」の興亡 | 2026-06-12-iran-proxies |
| 2026-06-14 | asia | 台湾海峡の「新常態」— 火を使わない包囲という戦略 | 2026-06-14-taiwan-strait |
| 2026-06-15 | europe | ドイツという「病人」の再来 — 欧州を牽引した国は、なぜ失速したか | 2026-06-15-germany-sick-man |
| 2026-06-15 | ai-tech | 「考えるAI」から「動くAI」へ — 自律エージェントの実力と限界 | 2026-06-15-ai-agents |
| 2026-06-15 | resources | 氷上のシルクロード — 温暖化が開いた航路と資源の地政学 | 2026-06-15-arctic-thaw |
| 2026-06-16 | asia | 世界で最も危険な国境 — カシミールが映す核時代の南アジア | 2026-06-16-india-pakistan |
| 2026-06-19 | security | 実戦を知った核 — 北朝鮮を変えた「ロシアの後ろ盾」 | 2026-06-19-north-korea-nuclear |
| 2026-06-20 | technology | 500ドルの空軍 — モスクワを襲ったドローン、その技術の中身 | 2026-06-20-drone-warfare |
| 2026-06-21 | security | サヘルの崩壊 — 西アフリカ連続クーデターとワグネルの「新植民地」 | 2026-06-21-sahel-collapse |
| 2026-06-22 | security | 国連は死んだのか — 安保理の機能不全と「ルールなき世界」 | 2026-06-22-un-security-council |
| 2026-06-23 | ukraine | ウクライナ、終わらない消耗戦 — 戦線・経済・疲弊する欧州 | 2026-06-23-war-of-attrition |
| 2026-06-24 | resources | 気候難民 2050年 — 国土を失う国々と「気候正義」の南北対立 | 2026-06-24-climate-refugees |
| 2026-06-26 | middle-east | ホルムズ海峡封鎖 — 世界の石油2割を握る「最後の蛇口」 | 2026-06-26-hormuz-strait |
| 2026-06-29 | ai-tech | スケーリングの終焉？ —AIは「大きくする」から「長く考える」へ | 2026-06-29-scaling-to-reasoning |
| 2026-07-01 | security | 見えない戦争 — AIが加速させる情報操作の世界地図 | 2026-07-01-information-warfare |
| 2026-07-14 | middle-east | 国家なき最大の民族 — クルド人はなぜ独立できないのか | 2026-07-14-kurds-stateless |
| 2026-07-15 | resources | 海底のゴールドラッシュ — ルールなき深海に、誰が最初に手を伸ばすか | 2026-07-15-deep-sea-mining |
| 2026-08-04 | security | 「裏庭」と呼ばれた大陸 — アメリカはなぜ、大統領を連れ去れたのか | 2026-08-04-latin-america-intervention |
| 2026-08-25 | technology | 「安全」という商品 — 街を覆うカメラと、記者を狙うソフト | 2026-08-25-surveillance-exports |
| 2026-08-26 | economy | 人口が減る国、増える国 — 国力の前提が崩れるとき | 2026-08-26-population-decline |
| 2026-08-27 | ukraine | ロシアはあと何年、戦争を続けられるのか — 戦時経済を数字で追う | 2026-08-27-russia-war-economy |
| 2026-08-28 | technology | 太陽を、地上で — 核融合はいつ、誰の手で電気になるか | 2026-08-28-fusion-race |
| 2026-08-30 | asia | デフレは終わったのか — 中国経済、12期ぶりのプラスの中身 | 2026-08-30-china-deflation |
| 2026-09-04 | resources | 38分後に届いた警報 — ヒマラヤは、誰も見ていない場所で崩れた | 2026-09-04-himalaya-glacier-collapse |
| 2026-09-06 | resources | 森は誰のものか — 「切る権利」をめぐる五つの正義 | 2026-09-06-forest-sovereignty |
| 2026-09-07 | middle-east | パレスチナ国家承認ラッシュ ―「国家」を誰が決めるのか | 2026-09-07-palestine-recognition |
| 2026-09-07 | elections | アメリカ、中絶をめぐる州境の分裂 — 全国共通の権利から、住む州で変わる権利へ | 2026-09-07-us-abortion-divide |
| 2026-09-08 | security | 逮捕状は、届かない — 国際刑事裁判所、内と外からの危機 | 2026-09-08-icc-crisis |
| 2026-09-15 | ai-tech | AIの代償を数字で見る ― 1つの質問が使う電力と水 | 2026-09-15-ai-power-water |
| 2026-09-16 | elections | 「引き裂かれたドイツ」— AfD支持拡大の実像と、対抗する側の主張 | 2026-09-16-afd-germany-divide |
| 2026-09-19 | immigration | 不法移民は、全員送還すべきか ― アメリカを割る三つの言い分 | 2026-09-19-us-deportation-divide |
| 2026-09-19 | technology | あなた用に作られた世界 — おすすめ表示は、人の考えまで変えるのか | 2026-09-19-algorithmic-bubble |
| 2026-09-20 | elections | 3議席の攻防 — アメリカ中間選挙、下院を分ける薄氷の差 | 2026-09-20-us-midterm-elections |
| 2026-09-22 | elections | 正しさの代償 — 強い確信ほど、人間関係は狭くなるという逆説 | 2026-09-22-cost-of-being-right |
| 2026-09-25 | asia | 台湾とホルムズ — 米中首脳会談、両国は何を求め、何を狙ったか | 2026-09-25-us-china-summit |

---

## 記事作成の手順・実装処方について

記事作成の**手順**（記事案の提案→Web調査→Midjourneyプロンプト出力→記事HTML作成→カテゴリ/ホーム更新→画像圧縮→コミット）、**実装担当のOpus/Sonnet使い分け**、**ヒーロー・head要素の実装処方**、**画像処理コマンド**、**Midjourneyプロンプト出力ルール**は、すべて **`/worldview-article` スキル**（`.claude/skills/worldview-article/SKILL.md`）に置く。

このCLAUDE.mdには、記事作成時に**守るべき事実・規約・索引**のみを置く（手順・コマンド・詳細CSSはスキル側）。新しい記事を作るときは `/worldview-article` を使う。

記事を追加したら、下記「既存記事一覧」表に1行追記すること（重複防止インデックスの維持）。

---

## デザインルール（必須）

守るべき原則のみを記載。ヒーロー・head要素・情報源リンクの**具体的なCSS処方は `/worldview-article` スキル**に委譲。

### 禁止事項
- **国名略語の禁止**：US・EU・UK等は使わず、アメリカ・欧州連合・イギリス等の日本語で表記
- **絵文字アイコンの禁止**：記事本文・見出し・ラベルに絵文字を使わない（チープに見える）
- **perspective-cardの色分け禁止**：`--card-color` のバリエーション指定はしない（全カード統一）
- **メタ欄はカテゴリ＋日付のみ**：ヒーローの `article-meta` 欄には**カテゴリのタグ1つと日付だけ**を置く。トピック名などの追加タグ（例「スケーリング則」）は入れない。
- **セクション間の相互参照の禁止**：本文に「（詳細は0X参照）」のような、セクション番号で他の節へ送る書き方をしない。読者に行き来を強いるうえ、記事が節の集合に見える。必要な事実はその場で短く言い切り、詳しく述べるのは一次情報源となる節だけにする。
- **本文で凝った比喩を使わない**：地の文は事実と論理をそのまま書く。「一社の製品が、世界地図に広がっていた」のような、語の結びつきが日本語として成立しない装飾表現を避ける（詳細・具体例は `/natural-japanese` スキル参照）。記事タイトル・セクション見出しは端的で印象に残る言い方をしてよい（例外）。

### カラー原則
- **ベース70%・メイン25%・アクセント5%** の比率を守る
- 色の使いすぎ禁止。対比・強調が必要な箇所のみ色を使う
- スタンスの色分けは `tone-meter` のクラス（`.tone-hostile` / `.tone-supportive`）で行う

### 記事レイアウト
- **文章量は「安全という商品」記事を基準にする（必須）**：`articles/technology/2026-08-25-surveillance-exports/index.html`（全998行、本文可読テキスト約9,600字）を今後の記事の分量上限の目安とする。過去記事にはこれより文章量の多いものもあるが、新規記事はこの水準を超えないよう意識して書く（読む負担を減らす方針）。書き終えたら明らかに超過していないか目視で確認し、超えていれば以下を優先して削る：①複数セクションで同じ事実（日付・金額・件数）を詳しく語り直している箇所（一次情報源となるセクションで一度だけ詳しく語り、他セクションは「詳細は0X参照」で短く参照する）②深掘り(05)のinsight-boxで他セクションと内容が重複するもの（3〜4個が目安）③くどい前置き・二重の説明。
- **6セクション構成（標準）**：① 論点（what + why が一体：定義・構造・本記事の問いを提示）② タイムライン ③ 各国・各勢力の視点 ④ データ・統計 ⑤ 深掘り（筆者個人の見解）⑥ 情報源
  - ①のセクション見出しラベルは**「論点」**で統一する（旧称「核心と問い」は廃止。2語の並列は読み手が関係性を推論する負荷が高いため、1語の熟語に統一）
  - この並び順を今後の全記事の標準とする。セクションIDは `sec-core / sec-timeline / sec-perspectives / sec-data / sec-analysis / sec-sources`
  - **深掘り(05)は「筆者個人の見解」を述べるセクションとする（必須）**：01〜04の事実を要約・再掲するのではなく、その事実にもとづく予想・見通しを書く。セクション冒頭に「※ ここから先は、ここまでの事実にもとづく筆者個人の見解です。」という断り書きを必ず入れる。深掘りは独立した見解として完結させる（相互参照の禁止は上記「禁止事項」のとおり全セクションに適用）。
- **perspective-card**：視点数は**記事ごとに最適なものを都度判断する（4〜6視点）**。「6ありき」にしない。対立が二項に近いトピックは4〜5でシャープに、当事者が多い多国間トピックは6が妥当。テーマの構造で決める。スタンスはテキストラベルと tone-meter で区別
  - **整合性の徹底**：採用した視点数を、記事の比較表・カテゴリページ／ホームの「N視点」バッジ・記事下部TOCメタの「N つの視点を掲載」まで実数で一致させる（バッジだけ6のまま等のズレを作らない）
- **画像構成**：ヒーロー1枚＋本文中4枚、計5枚。うち最低1枚は近接・接写
  - 本文中の配置場所は以下の4か所に固定する（全て同格）：
    - `sec-core`（01）末尾 — factor-grid の直後
    - `sec-timeline`（02）末尾
    - `sec-perspectives`（03）末尾 — 比較表の下
    - `sec-analysis`（05）末尾 — insight-box群の下
  - `sec-data`（04）と `sec-sources`（06）には本文中画像を置かない
  - **各ポジションの画像は、そのセクションの内容と関連するものにする**（01なら記事の核心テーマ、02なら時代背景・現場、03なら視点の対立、05なら将来像・帰結など）
  - ファイル名規則：`{スラッグ}-{内容キーワード}.jpg`（全ポジション統一）
- **Chart.js**：CDN `chart.js@4.4.0` を使用。記事ごとに1〜2チャート
  - **チャートの数値は出典の記載値のみを使う（必須）**：グラフに入れる全数値について、`data:` の直前に出典と原記述をコメントで記録する。**出典に無い数値は使用禁止**——定性的な記述（例「陸上埋蔵量を上回る」）からの逆算、見栄えのための調整値、最新値以外を補完した推移データは作らない。裏付けが取れないならグラフにせず本文の記述で伝える。
- **主観的な強弱を数値の百分率で示さない（必須）**：perspective-cardの「賛否の強さ」「推進度」など、測定不能な主観的評価を `tone-meter` 等でパーセンテージ表示しない。出典のないパーセンテージは、チャートの数値捏造と同じ「偽の精度」の問題を生む。強弱を示したい場合はテキストのラベル（例「強く推進」「慎重」）で表現し、数値化しない。
- **情報源は必ず実URLのリンクにする（必須）**：`sec-sources`（06）の各カードは `<a class="source-item" href="https://..." target="_blank" rel="noopener">` で実装する（CSSスニペットはスキル参照）。
  - URLは**実在を確認した一次情報**を貼る（憶測・推測のURLは404を生むので禁止）。調査時の `WebSearch` で得た正規URLを使う。
  - `source-name` と貼るURLの内容を一致させる（媒体名と中身がズレないように）。
- **ヒーローは案X型が標準**：フォトリアル画像を背景に敷き、`article-meta`→`article-title`→宣言文 `.hero-deck`→数字ピル `.hero-pills` を重ねる。宣言文のアクセント・数字ピルの色は**カテゴリのテーマ色 `var(--accentN)` に合わせる**（直書き禁止）。生成AI画像には `.hero-credit` で「イメージ画像（生成AI）」を明示。→ 具体的なCSS処方・数値は `/worldview-article` スキル。
- **head要素（SEO・OGP）は全ページ必須**：`description`／`canonical`（絶対URL・ディレクトリ形）／`favicon`／OGP／Twitterカードを `<meta name="viewport">` 直後に入れる。`og:image`・`twitter:image` は絶対URL。→ 具体的なテンプレは `/worldview-article` スキル。
- **「この記事の問い」ボックスは左罫線を使わない**：淡い背景ウォッシュ＋薄い縁取り＋背後からの柔らかい光（box-shadowのグロー）で立体感を出す。外側マージンは上下 `3.3rem`。→ 具体的なCSS処方は `/worldview-article` スキル。
- **タイムラインは「フラット・日付見出し統合」型**：`.timeline-item` の区切りは余白のみ（`border-bottom` なし）。`timeline-content` はカード化せず、日付とその日の出来事の見出しを1行に統合し、`<h4>` やリアクションチップは使わない。→ 具体的なCSS・HTML処方は `/worldview-article` スキル。

### ヘッダーナビ（単一ソース / 必須）

ナビは **`assets/js/main.js` の配列1か所が唯一の定義**。各ページの `<nav class="site-nav">` は**空にしておく**（`<nav class="site-nav"></nav>`）。JSが中身を生成する。

- **デスクトップ**：ホーム＋主要カテゴリ（`primaryKeys`）＋「すべて ≡」を表示。「すべて」で全カテゴリのドロワーを開く。
- **モバイル**：ハンバーガー → 全カテゴリ（`allCategories`）のドロワー。
- **現在地ハイライト**はURLから自動。手書きの `style="color:var(--text)"` は付けない。
- **新規ページ作成時**は手書きナビを書かない（空の `<nav class="site-nav"></nav>` だけ置く）。カテゴリ追加時は `main.js` の `allCategories`（必要なら `primaryKeys`）に1行追加するだけ。
- カテゴリ表示名は `allCategories` に一本化（表記ゆれ禁止。例：「移民政策」で統一）。

---

## 運用上の絶対ルール

- **コミット・公開（git commit / git push）の前に必ずユーザーに確認を取ること。** 無断で実行しない。
- **公開前にファクトチェック工程を必ず通すこと。** 未実施のままコミットしない。手順は `/fact-check` スキル（検証はサブエージェントに委譲し、修正方針はユーザーと決める）。
- **記事を追加・修正したら検索インデックスを再生成すること。** `uv run python tools/build_search_index.py` を実行して `assets/data/articles.js` を更新する。このファイルは生成物なので手で編集しない（正はカテゴリページの記事カードと記事HTMLの見出し）。再生成を忘れると新しい記事がサイト内検索に出てこない。
- **検索インデックスを JSON 形式に変えないこと。** `articles.js` がグローバル変数への代入になっているのは、`file://` でHTMLを直接開いたときブラウザが `fetch()` をCORSでブロックするため。JSONにすると、ローカルでHTMLをダブルクリックして確認したときだけ検索が動かなくなる。
- **公開前に日本語の自然さチェック工程を必ず通すこと。** 未実施のままコミットしない。手順は `/natural-japanese` スキル（記事HTML作成直後・カテゴリ/ホーム転記前に実施。意味検査はサブエージェントに委譲）。

---

## GitHub Pages 運用メモ

- ブランチ：`main`（直接公開）
- コミット後 `git push origin main` で即時反映
- 公開確認は数分後に https://kei-code.github.io/worldview/ にアクセス
- リポジトリはPublicだが、直リンクを知っている人のみアクセス可能（検索インデックスは未登録）
