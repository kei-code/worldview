...
- **6セクション構成（標準）**：① 論点（what + why が一体：定義・構造・本記事の問いを提示）② タイムライン ③ 各国・各勢力の視点 ④ データ・統計 ⑤ 深掘り（筆者個人の見解）⑥ 情報源
  ...
  - **深掘り(05)は「筆者個人の見解」を述べるセクションとする（必須）**：01〜04の事実を要約・再掲するのではなく、その事実にもとづく予想・見通しを書く。セクション冒頭に「※ ここから先は、ここまでの事実にもとづく筆者個人の見解です。」という断り書きを必ず入れる。他セクションへの「（詳細は0X参照）」は付けない（深掘りは独立した見解として完結させる）。
