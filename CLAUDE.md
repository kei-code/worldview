...
- **画像構成**：ヒーロー1枚＋本文中4枚（**sec-core専用1枚**＋残り3枚）、計5枚。うち最低1枚は近接・接写
  - **sec-core（01 核心と問い）には必ず専用画像を1枚掲載する**。セクション末尾の factor-grid の直後に `<figure class="article-figure">` で配置する
  - ファイル名規則：`{スラッグ}-core-{内容キーワード}.jpg`（例：`ai-agents-core-thinking-vs-acting.jpg`）
...
- **構成**：5ポジション（ヒーロー・**sec-core専用**・本文中①・本文中②・本文中③）× 各2案 = 計10プロンプト
