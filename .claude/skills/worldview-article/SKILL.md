...
description: WorldView記事の新規作成フロー。記事案の提案→Web調査→Midjourneyプロンプト出力→記事HTML作成→カテゴリ/ホーム更新→画像圧縮→検索インデックス再生成→コミットまでの手順と、ヒーロー・head要素・画像処理の実装処方をまとめる。新しい記事を書く・追加するとき、記事の画像を配置・圧縮するときに使う。
---
...
10. **検索インデックスの再生成（必須）**：`uv run python tools/build_search_index.py` を実行する。カテゴリページの記事カードと記事の見出しから `assets/data/articles.js` を作り直す工程で、これを飛ばすと新しい記事がサイト内検索に出てこない。記事カードの必須項目が欠けているとスクリプトがエラーで止まるので、止まった場合はカテゴリページ側を直してから再実行する
11. **コミット＆公開**：ユーザー確認後に `git commit` → `git push`（無断実行しない＝CLAUDE.md「運用上の絶対ルール」）
